(ns nokia_vonage.main-test
  "Contract + behavioral test for the nokia_vonage-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "nokia_vonage-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'nokia_vonage.main-test)]
