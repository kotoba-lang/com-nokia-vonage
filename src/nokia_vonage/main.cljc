(ns nokia_vonage.main
  "Kotodama WASM entrypoint for the nokia_vonage clean-room actor (L4) — Clojure port.
  ...
  facts (`nokia_vonage.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "nokia_vonage")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"nokia_vonage.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "nokia_vonage-compat" :tier tier :entities entities} 200])
...
(defn start! [] :nokia_vonage-compat/ready)
